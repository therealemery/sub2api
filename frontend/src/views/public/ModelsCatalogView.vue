      ...
      <section class="catalog-workspace" aria-labelledby="catalog-results-title">
        ...
        <div class="catalog-grid">
          <aside class="filter-rail" :class="{ 'filter-rail-open': filtersOpen }">
            ...
            <fieldset>
              <legend>{{ t('publicModels.capability') }}</legend>
              <label><input v-model="filters.capability" type="radio" value="" />{{ t('publicModels.allCapabilities') }}</label>
              <label v-for="capability in capabilities" :key="capability"><input v-model="filters.capability" type="radio" :value="capability" />{{ capability }}</label>
            </fieldset>
          </aside>
          ...
          <div class="catalog-results">
            <div class="results-toolbar">
              ...
              <label>{{ t('publicModels.sort') }}
                <select v-model="filters.sort">
                  ...
                  <option value="price">{{ t('publicModels.sortPrice') }}</option>
                </select>
              </label>
            </div>
            ...
            <div v-else-if="filteredModels.length" class="model-card-grid">
              <router-link v-for="model in filteredModels" :key="`${model.platform}:${model.modelId}`" :to="`/models/${model.slug}`" class="model-card">
                <div class="model-art"><img :src="model.artwork" alt="" loading="lazy" /><span v-if="model.featured">{{ model.featuredBadge || t('publicModels.featured') }}</span></div>
                <div class="model-card-body">
                  ...
                  <div class="model-card-footer"><span>{{ priceLabel(model) }}</span><strong>{{ t('publicModels.viewModel') }} <Icon name="arrowRight" size="xs" /></strong></div>
                </div>
              </router-link>
...
import { buildModelCatalog, filterModelCatalog, type ModelCatalogEntry, type CatalogFilters } from '@/data/modelCatalog'
...
const filters = reactive<CatalogFilters>({ query: '', provider: '', capability: '', sort: 'featured' })

const providers = computed(() => [...new Set(models.value.map((model) => model.provider))].sort())
const capabilities = computed(() => [...new Set(models.value.flatMap((model) => model.capabilities))].sort())
...
function priceLabel(model: ModelCatalogEntry): string {
  const amount = model.price?.input ?? model.price?.perRequest
  if (amount == null) return t('publicModels.priceUnavailable')
  const unit = model.price?.perRequest != null && model.price.input == null ? t('publicModels.perRequest') : t('publicModels.perMillion')
  return `${t('publicModels.from')} $${amount.toLocaleString(undefined, { maximumFractionDigits: 4 })} ${unit}`
}
</script>
