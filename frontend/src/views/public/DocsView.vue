      ...
      <div class="docs-shell">
        ...
        <article class="docs-content">
          ...
          <section id="video-api" class="docs-section video-docs-section">
            ...
            <div class="video-model-tabs" role="tablist" :aria-label="t('publicDocs.videoModelLabel')">
              <button v-for="option in videoModelOptions" :key="option" type="button" role="tab" :aria-selected="selectedVideoModel === option" @click="selectedVideoModel = option">{{ option }}</button>
            </div>
            <div class="video-docs-grid">
              <div><span class="video-step">01</span><h3>{{ t('publicDocs.videoCreateTitle') }}</h3><p>{{ t(selectedVideoModel === 'MiniMax-H3' ? 'publicDocs.videoCreateDescriptionH3' : 'publicDocs.videoCreateDescriptionWan') }}</p><pre><code>{{ videoCreateExample }}</code></pre></div>
...
const videoModelOptions = ['MiniMax-H3', 'wan3.0-video', 'wan3.0-video-prime'] as const
const selectedVideoModel = ref<typeof videoModelOptions[number]>('MiniMax-H3')
...
const videoCreateExample = computed(() => selectedVideoModel.value === 'MiniMax-H3' ? h3CreateExample : `curl "${baseUrl}/videos" \\
  ...
  -d '{
    ...
    "prompt": "Video 1 follows Image 1 through a cinematic city scene.",
    "duration": 5,
    "resolution": "480P",
    "ratio": "16:9",
    "audio": true,
...
.video-model-tabs{display:flex;flex-wrap:wrap;gap:8px;margin-top:30px}.video-model-tabs button{min-height:36px;border:1px solid #d8d8d8;border-radius:999px;background:#fff;padding:0 13px;color:#666;font:inherit;font-size:11px;cursor:pointer}.video-model-tabs button[aria-selected="true"]{border-color:#171717;background:#171717;color:#fff}
