        ...
        <section class="detail-overview">
          ...
          <div class="detail-content-grid">
            <div>
              ...
              <section v-if="pricingCondition && formattedPricingCondition" class="pricing-condition" aria-labelledby="pricing-condition-title">
                <span class="section-label">{{ t('publicModels.pricingCondition.label') }}</span>
                <h2 id="pricing-condition-title">{{ pricingConditionName }}</h2>
                <p>{{ t('publicModels.pricingCondition.baseOutsidePeak') }}</p>
                <dl>
                  <div><dt>{{ t('publicModels.pricingCondition.schedule') }}</dt><dd>{{ t('publicModels.pricingCondition.weekdays') }} · {{ t('publicModels.pricingCondition.beijingTime', { windows: formattedPricingCondition.beijingWindows }) }}</dd></div>
                  <div><dt>{{ t('publicModels.pricingCondition.utc') }}</dt><dd>{{ formattedPricingCondition.utcWindows }}</dd></div>
                  <div><dt>{{ t('publicModels.pricingCondition.multiplier') }}</dt><dd>{{ formattedPricingCondition.multiplier }}</dd></div>
                </dl>
                <p>{{ t('publicModels.pricingCondition.halfOpen') }}</p>
                <p>{{ t('publicModels.pricingCondition.usageAudit') }}</p>
              </section>
            ...
            <aside class="pricing-panel">
              ...
              <template v-else-if="detailPricingSource">
                ...
                <p v-if="detailPricingSource.status !== 'paid'" class="pricing-state">
                  {{ detailPricingSource.status === 'free' ? t('publicModels.free') : t('publicModels.notPublished') }}
                </p>
                <div v-else class="pricing-tier">
                  <div v-if="pricingTierOptions.length > 1" class="pricing-tier-selector" role="group" :aria-label="t('publicModels.pricingTier')">
                    <button
                      v-for="tier in pricingTierOptions"
                      :key="tier.id"
                      type="button"
                      class="pricing-tier-option"
                      :class="{ active: selectedPricingTierId === tier.id }"
                      :aria-pressed="selectedPricingTierId === tier.id"
                      @click="selectPricingTier(tier.id)"
                    >
                      {{ tier.label }}
                    </button>
                  </div>
                  ...
                  <Transition name="motion-fade" mode="out-in">
                    <div :key="pricingTierKey(model, selectedPricingTierId, selectedPricing)" class="pricing-tier-values">
                      <table>
                        ...
                        <tbody>
                          <tr v-for="row in pricingRows(selectedPricing, detailPricingSource.multiplier)" :key="row.key">
...
import { formatRequestPricingCondition, requestPricingConditionForModel } from '@/data/requestPricingConditions'
import {
  buildModelCatalog,
...
const selectedPricingTierIds = ref<Record<string, string>>({})
const requestPricingConditions = ref<PublicRequestPricingConditionRule[]>([])
...
const pricingCondition = computed(() => model.value
  ? requestPricingConditionForModel(requestPricingConditions.value, model.value.modelId)
  : null)
const formattedPricingCondition = computed(() => pricingCondition.value
  ? formatRequestPricingCondition(pricingCondition.value)
  : null)
const pricingConditionName = computed(() => {
  const rule = pricingCondition.value
  if (!rule) return ''
  return String(locale.value).toLowerCase().startsWith('zh') ? rule.name_zh : rule.name_en
})
const pricingTierOptions = computed(() => {
  const source = detailPricingSource.value
  if (!source) return []
  return [
    { id: 'base', label: t('publicModels.pricingTierBase'), official: source.official },
    ...source.tiers.map((tier) => ({ id: tier.id, label: pricingTierLabel(tier), official: tier.official })),
  ]
})
const selectedPricingTierId = computed(() => {
  const identity = modelIdentity(model.value)
  const selected = selectedPricingTierIds.value[identity] ?? 'base'
  return pricingTierOptions.value.some((tier) => tier.id === selected) ? selected : 'base'
})
const selectedPricingTier = computed(() => pricingTierOptions.value.find((tier) => tier.id === selectedPricingTierId.value) ?? pricingTierOptions.value[0])
const selectedPricingTierLabel = computed(() => selectedPricingTier.value?.label ?? t('publicModels.pricingTierBase'))
const selectedPricing = computed<OfficialTokenPricing>(() => selectedPricingTier.value?.official ?? { input: null, cachedInput: null, output: null })
...
async function loadCatalog() {
  try {
    ...
    entries.value = buildModelCatalog(config)
    requestPricingConditions.value = config.request_pricing_conditions ?? []
  } catch {
    entries.value = buildModelCatalog()
    requestPricingConditions.value = []
  } finally {
    loading.value = false
  }
}
...
function modelIdentity(entry: ModelCatalogEntry | null | undefined): string {
  return entry ? `${entry.platform}:${entry.modelId}` : ''
}

function selectPricingTier(tierId: string) {
  const identity = modelIdentity(model.value)
  if (!identity) return
  selectedPricingTierIds.value = { ...selectedPricingTierIds.value, [identity]: tierId }
}

function pricingTierLabel(tier: OfficialPricingTier): string {
  const minOperator = tier.minInclusive ? '≥' : '>'
  const min = tier.minInputTokens.toLocaleString('en-US')
  if (tier.maxInputTokens == null) {
    return t('publicModels.pricingTierMinimum', { operator: minOperator, count: min })
  }
  return t('publicModels.pricingTierRange', {
    minOperator,
    min,
    maxOperator: tier.maxInclusive ? '≤' : '<',
    max: tier.maxInputTokens.toLocaleString('en-US'),
  })
}
...
.pricing-notes{display:grid;gap:8px;margin:0;padding:18px 0 0 18px;border-top:1px solid #e8e8e8;color:#777;font-size:11px;line-height:1.5}.pricing-condition{display:grid;gap:15px;border:1px solid #d8d8d8;border-radius:15px;background:#fff;padding:24px}.pricing-condition h2{margin:0;font-size:24px;font-weight:570;letter-spacing:-.03em}.pricing-condition p{max-width:720px;margin:0;color:#666;font-size:13px;line-height:1.65}.pricing-condition dl{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));margin:4px 0;border-top:1px solid #e8e8e8;border-bottom:1px solid #e8e8e8}.pricing-condition dl>div{display:grid;gap:8px;padding:17px 16px;border-right:1px solid #e8e8e8}.pricing-condition dl>div:first-child{padding-left:0}.pricing-condition dl>div:last-child{border-right:0}.pricing-condition dt{color:#777;font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:.07em}.pricing-condition dd{margin:0;font-size:13px;font-weight:560;line-height:1.5}
.pricing-tier-selector{display:flex;flex-wrap:wrap;gap:6px}.pricing-tier-option{min-height:34px;border:1px solid #ddd;border-radius:999px;background:#fff;padding:0 11px;color:#666;font:inherit;font-size:10px;line-height:1.2;cursor:pointer;transition:border-color var(--motion-fast) var(--ease-standard),background var(--motion-fast) var(--ease-standard),color var(--motion-fast) var(--ease-standard)}.pricing-tier-option:hover,.pricing-tier-option.active{border-color:#171717;background:#171717;color:#fff}.pricing-tier-option:focus-visible{outline:2px solid #171717;outline-offset:2px}.pricing-state{margin:0;border-top:1px solid #e8e8e8;padding:22px 0 4px!important;color:#171717!important;font-size:22px!important;font-weight:600}.pricing-state+.pricing-notes{margin-top:18px}
@media(max-width:800px){.detail-hero,.detail-overview,.related-section{width:calc(100% - 32px)}.detail-hero-grid{grid-template-columns:1fr;gap:42px;margin-top:40px}.detail-art{grid-row:1}.detail-identity h1{font-size:clamp(52px,15vw,76px)}.metadata-grid{grid-template-columns:1fr}.metadata-grid article{min-height:96px;border-right:0;border-bottom:1px solid #ddd}.metadata-grid article:last-child{border-bottom:0}.detail-content-grid{grid-template-columns:1fr}.pricing-panel{position:static;grid-row:1}.pricing-condition dl{grid-template-columns:1fr}.pricing-condition dl>div{padding:14px 0;border-right:0;border-bottom:1px solid #e8e8e8}.pricing-condition dl>div:last-child{border-bottom:0}.related-grid{grid-template-columns:1fr}.related-heading{align-items:flex-start;flex-direction:column;gap:18px}}
