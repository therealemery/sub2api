...
const levelBadgeClass = (level: string) => {
  ...
  return 'bg-gray-100 text-gray-700 dark:bg-[var(--bg-surface)]/10 dark:text-gray-300'
}
...
<template>
  <section class="rounded-lg border border-gray-200 bg-[var(--bg-surface)] p-4 border-[var(--border-default)] bg-[var(--bg-surface-alt)]">
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
      <div>
        <h3 class="text-sm font-bold text-gray-900 dark:text-[var(--text-inverse)]">系统日志</h3>
        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">默认按最新时间倒序，支持筛选搜索与按条件清理。</p>
      </div>
      <div class="flex flex-wrap items-center gap-2 text-xs">
        <span class="rounded-md bg-gray-100 px-2 py-1 text-gray-700 bg-[var(--bg-surface-alt)] dark:text-gray-200">队列 {{ health.queue_depth }}/{{ health.queue_capacity }}</span>
        <span class="rounded-md bg-gray-100 px-2 py-1 text-gray-700 bg-[var(--bg-surface-alt)] dark:text-gray-200">写入 {{ health.written_count }}</span>
    ...
    <div class="mb-4 rounded-lg border border-gray-200 bg-gray-50 p-3 border-[var(--border-default)] bg-[var(--bg-surface-alt)]">
