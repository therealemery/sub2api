...
const healthScoreColor = computed(() => {
  ...
  if (score >= 90) return '#287a4b' // green
  if (score >= 60) return '#986b16' // yellow
  return '#ef4444' // red
})
    ...
    <div class="ops-toolbar flex flex-wrap items-center justify-between gap-4 border-b border-[var(--border-default)] pb-4">
      ...
      <div class="flex flex-wrap items-center gap-3">
        <template v-if="!props.fullscreen">
          ...
          <div class="mx-1 hidden h-4 w-[1px] bg-gray-200 bg-[var(--bg-surface-alt)] sm:block"></div>
        ...
        <div v-if="!props.fullscreen" class="mx-1 hidden h-4 w-[1px] bg-gray-200 bg-[var(--bg-surface-alt)] sm:block"></div>
    ...
    <div v-if="overview" class="grid grid-cols-1 gap-6 lg:grid-cols-12">
            ...
            <div class="relative flex items-center justify-center">
              <svg :width="circleSize" :height="circleSize" class="-rotate-90 transform">
                <circle
                  ...
                  fill="transparent"
                  class="text-gray-200 text-[var(--text-muted)]"
                  stroke="currentColor"
                />
                <circle
                  :cx="circleSize / 2"
                  ...
                  class="transition-colors duration-1000 ease-out"
                />
              </svg>
          ...
          <div :class="['ops-health-card rounded-lg border border-[var(--border-default)] bg-[var(--bg-surface-alt)]', props.fullscreen ? 'p-6' : 'p-4']">
            ...
            <div class="mt-3 space-y-3">
              <div
                ...
                :key="idx"
                class="rounded-md border border-[var(--border-default)] bg-[var(--bg-surface)] p-3"
              >
      ...
      <div class="ops-metric-grid grid h-full grid-cols-1 content-center gap-4 sm:grid-cols-2 lg:col-span-7 lg:grid-cols-3">
        ...
        <div class="ops-metric-card rounded-lg border border-[var(--border-default)] bg-[var(--bg-surface-alt)] p-4" style="order: 2;">
          ...
          <div class="mt-3 h-2 w-full overflow-hidden rounded-full bg-gray-200 bg-[var(--bg-surface-alt)]">
            <div class="h-full transition-colors" :class="getSLAThresholdLevel(slaPercent) === 'critical' ? 'bg-red-500' : getSLAThresholdLevel(slaPercent) === 'warning' ? 'bg-yellow-500' : 'bg-green-500'" :style="{ width: `${Math.max((slaPercent ?? 0) - 90, 0) * 10}%` }"></div>
          </div>
