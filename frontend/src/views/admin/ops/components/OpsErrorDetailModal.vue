    ...
    <div v-else class="space-y-6 p-6">
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div class="rounded-lg bg-gray-50 p-4 bg-[var(--bg-surface-alt)]">
          ...
          <div class="mt-1 break-all font-mono text-sm font-medium text-gray-900 dark:text-[var(--text-inverse)]">
            {{ requestId || '—' }}
          </div>
        </div>
        ...
        <div class="rounded-lg bg-gray-50 p-4 bg-[var(--bg-surface-alt)]">
          ...
          <div class="mt-1 text-sm font-medium text-gray-900 dark:text-[var(--text-inverse)]">
        ...
        <div class="rounded-lg bg-gray-50 p-4 bg-[var(--bg-surface-alt)]">
          ...
          <div class="mt-1 truncate text-sm font-medium text-gray-900 dark:text-[var(--text-inverse)]" :title="detail.message">
            {{ detail.message || '—' }}
          </div>
        </div>
      </div>
      ...
      <div class="rounded-lg bg-gray-50 p-6 bg-[var(--bg-surface-alt)]">
        <h3 class="text-sm font-black uppercase tracking-wider text-gray-900 dark:text-[var(--text-inverse)]">{{ t('admin.ops.errorDetail.responseBody') }}</h3>
        <pre class="mt-4 max-h-[520px] overflow-auto rounded-lg border border-gray-200 bg-[var(--bg-surface)] p-4 text-xs text-gray-800 border-[var(--border-default)] bg-[var(--bg-surface-alt)] dark:text-gray-100"><code>{{ prettyJSON(primaryResponseBody || '') }}</code></pre>
      </div>
      ...
      <div v-if="showUpstreamList" class="rounded-lg bg-gray-50 p-6 bg-[var(--bg-surface-alt)]">
        <div class="flex flex-wrap items-center justify-between gap-2">
          <h3 class="text-sm font-black uppercase tracking-wider text-gray-900 dark:text-[var(--text-inverse)]">{{ t('admin.ops.errorDetails.upstreamErrors') }}</h3>
        ...
        <div v-else class="mt-4 space-y-3">
          ...
          >
            <div class="flex flex-wrap items-center justify-between gap-2">
              <div class="text-xs font-black text-gray-900 dark:text-[var(--text-inverse)]">
                #{{ idx + 1 }}
                <span v-if="ev.type" class="ml-2 rounded-md bg-gray-100 px-2 py-0.5 font-mono text-[10px] font-bold text-gray-700 bg-[var(--bg-surface-alt)] dark:text-gray-200">{{ ev.type }}</span>
              </div>
              <div class="flex items-center gap-2">
                ...
                <button
                  type="button"
                  class="inline-flex items-center gap-1.5 rounded-md px-1.5 py-1 text-[10px] font-bold text-[var(--accent)] hover:bg-[var(--bg-subtle)] disabled:cursor-not-allowed disabled:opacity-60 text-[var(--accent)] dark:hover:bg-dark-700"
            ...
            <div v-if="ev.message" class="mt-3 break-words text-sm font-medium text-gray-900 dark:text-[var(--text-inverse)]">{{ ev.message }}</div>

            <pre
              v-if="expandedUpstreamDetailIds.has(ev.id)"
              class="mt-3 max-h-[240px] overflow-auto rounded-lg border border-gray-200 bg-gray-50 p-3 text-xs text-gray-800 border-[var(--border-default)] bg-[var(--bg-surface-alt)] dark:text-gray-100"
