    ...
    <div :class="['rounded-lg bg-[var(--bg-surface)] ring-1 ring-gray-900/5 bg-[var(--bg-surface-alt)] dark:ring-dark-700', props.fullscreen ? 'p-8' : 'p-6']">
      <div class="flex flex-wrap items-center justify-between gap-4 border-b border-gray-100 pb-4 border-[var(--border-default)]">
        <div class="space-y-2">
          ...
          <div class="h-3 w-80 animate-pulse rounded bg-gray-100 bg-[var(--bg-surface-alt)]"></div>
        </div>
        <div v-if="!props.fullscreen" class="flex flex-wrap items-center gap-3">
          <div class="h-9 w-[140px] animate-pulse rounded-lg bg-gray-200 bg-[var(--bg-surface-alt)]"></div>
          <div class="h-9 w-[160px] animate-pulse rounded-lg bg-gray-200 bg-[var(--bg-surface-alt)]"></div>
          <div class="h-9 w-[150px] animate-pulse rounded-lg bg-gray-200 bg-[var(--bg-surface-alt)]"></div>
          <div class="h-9 w-9 animate-pulse rounded-lg bg-gray-200 bg-[var(--bg-surface-alt)]"></div>
          <div class="h-9 w-28 animate-pulse rounded-lg bg-gray-200 bg-[var(--bg-surface-alt)]"></div>
          <div class="h-9 w-28 animate-pulse rounded-lg bg-gray-200 bg-[var(--bg-surface-alt)]"></div>
          <div class="h-9 w-9 animate-pulse rounded-lg bg-gray-200 bg-[var(--bg-surface-alt)]"></div>
        </div>
      </div>
      ...
      <div class="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-12">
        <div class="rounded-lg bg-gray-50 p-4 bg-[var(--bg-surface-alt)] lg:col-span-5">
          <div class="grid h-full grid-cols-1 gap-6 md:grid-cols-[200px_1fr] md:items-center">
            <div class="h-28 animate-pulse rounded-lg bg-gray-100 bg-[var(--bg-surface-alt)]"></div>
            <div class="space-y-4">
              <div class="h-4 w-32 animate-pulse rounded bg-gray-200 bg-[var(--bg-surface-alt)]"></div>
              <div class="grid grid-cols-2 gap-3">
                <div v-for="i in 4" :key="i" class="h-14 animate-pulse rounded-lg bg-gray-100 bg-[var(--bg-surface-alt)]"></div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="lg:col-span-7">
          <div class="grid h-full grid-cols-1 content-center gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <div v-for="i in 6" :key="i" class="h-20 animate-pulse rounded-lg bg-gray-50 bg-[var(--bg-surface-alt)]"></div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div :class="['rounded-lg bg-[var(--bg-surface)] ring-1 ring-gray-900/5 bg-[var(--bg-surface-alt)] dark:ring-dark-700', props.fullscreen ? 'p-8' : 'p-6']">
      <div class="flex flex-wrap items-center justify-between gap-4">
        ...
        <div v-if="!props.fullscreen" class="flex flex-wrap items-center gap-2">
          <div class="h-9 w-[140px] animate-pulse rounded-lg bg-gray-200 bg-[var(--bg-surface-alt)]"></div>
          <div class="h-9 w-[120px] animate-pulse rounded-lg bg-gray-200 bg-[var(--bg-surface-alt)]"></div>
          <div class="h-9 w-[120px] animate-pulse rounded-lg bg-gray-200 bg-[var(--bg-surface-alt)]"></div>
        </div>
      </div>

      <div class="mt-6 space-y-3">
        <div v-for="i in 6" :key="i" class="flex items-center justify-between gap-4 rounded-lg bg-gray-50 p-4 bg-[var(--bg-surface-alt)]">
          <div class="flex-1 space-y-2">
            <div class="h-3 w-56 animate-pulse rounded bg-gray-200 bg-[var(--bg-surface-alt)]"></div>
            <div class="h-3 w-80 animate-pulse rounded bg-gray-100 bg-[var(--bg-surface-alt)]"></div>
          </div>
          <div class="h-7 w-20 animate-pulse rounded-lg bg-gray-200 bg-[var(--bg-surface-alt)]"></div>
        </div>
      </div>
    </div>
  </div>
