  ...
  <AppLayout>
    ...
    >
      <div v-if="overviewLoading" class="flex justify-center py-8">
        <div class="h-6 w-6 animate-spin rounded-full border-2 border-[var(--border-focus)] border-t-transparent"></div>
      </div>
...
const UserCell = defineComponent({
  ...
  setup(cellProps, { emit }) {
    return () => h('div', { class: 'space-y-0.5' }, [
      ...
      h('div', { class: 'max-w-56 truncate text-sm text-gray-500 text-[var(--text-muted)]' }, cellProps.username || '-'),
    ])
  },
})
  ...
  setup(amountProps) {
    return () => {
      ...
      if (value === null || value === undefined) {
        return h('span', { class: 'text-sm text-gray-400 text-[var(--text-muted)]' }, '-')
      }
...
const OverviewStat = defineComponent({
  ...
  setup(statProps) {
    return () => h('div', { class: 'rounded-lg border border-gray-100 bg-[var(--bg-surface)] p-3 border-[var(--border-default)] bg-[var(--bg-surface-alt)]' }, [
