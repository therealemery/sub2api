export default {
  ...
  usage: {
    ...
    customerRate: '客户倍率',
    timeCondition: '时段条件',
    conditionMultiplier: '时段倍率',
    pricingRule: '计费规则',
    pricingEffectiveAt: '计费生效时间（UTC）',
    original: '原始',
    billed: '计费',
  ...
  publicModels: {
    ...
    pricingCondition: {
      label: '时段计费',
      baseOutsidePeak: '上方展示的 OwnAPI 基础价格适用于非高峰时段。',
      schedule: '高峰时段',
      weekdays: '周一至周五',
      beijingTime: '北京时间 {windows}',
      utc: '对应 UTC',
      multiplier: '高峰倍率',
      halfOpen: '开始时间包含在内，结束时间不包含在内。',
      usageAudit: '使用记录会标明本次请求采用标准 1× 还是高峰 2× 倍率。',
    },
    officialEightyPercent: '官方价 8 折',
    officialSeventyFivePercent: '官方价 75 折',
    cacheWrite: '缓存写入',
    perSecond: '/ 秒',
    perSecondBilling: '按生成时长计费',
    resolution: '分辨率',
    videoPriceEstimate: '视频按生成时长计费；价格表展示每个模型和分辨率对应的官网标价与 OwnAPI 价格。',
    viewProtocol: '查看接口协议',
    videoGenerator: { eyebrow: '登录用户专属', title: '生成 {model} 视频', apiKey: 'OwnAPI API Key', prompt: '提示词', promptPlaceholder: '描述画面、动作、镜头和风格…', duration: '视频时长', resolution: '分辨率', ratio: '画面比例', seed: '随机种子', audio: '生成音频', promptExtend: '智能改写提示词', watermark: '添加水印', referenceUrl: '参考图 URL（可选）', referenceFile: '上传参考图（可选）', firstFrameUrl: '首帧 URL（可选）', firstFrameFile: '上传首帧（可选）', lastFrameUrl: '尾帧 URL（可选）', lastFrameFile: '上传尾帧（可选）', referenceVideoUrl: '参考视频 URL（可选）', referenceVideoFile: '上传参考视频（可选）', referenceAudioUrl: '参考音频 URL（可选）', referenceAudioFile: '上传参考音频（可选）', h3ReferenceVideoUnavailable: 'MiniMax H3 参考视频暂不可用，参考图仍可正常使用。', h3VideoDuration: 'MiniMax H3 参考视频时长必须为 2–15 秒。', invalidH3Video: '无法读取 MiniMax H3 参考视频时长。', submit: '生成视频', submitting: '正在创建任务…', status: '任务状态', download: '打开或下载视频', failed: '视频请求失败' },
    ...
    families: {
      ...
      minimax: { summary: '覆盖长上下文智能与高质量视频生成。', description: 'MiniMax 模型家族支持推理、软件任务以及带参考图的文本生成视频工作流。' },
      wan: { summary: '阿里云万相多模态视频生成。', description: '支持文本、首尾帧、参考图、参考视频与参考音频驱动的高质量视频工作流。' },
  ...
  publicDocs: {
    eyebrow: '从这里开始', title: 'OwnAPI 使用文档', description: 'OwnAPI 快速开始、OpenAI 兼容调用、模型发现、集成方式与生产环境指南。', navigation: '文档目录', onThisPage: '本页内容', copyPage: '复制页面链接', copied: '已复制', quickStart: '快速开始', quickStartDescription: '创建 API Key，并在几分钟内完成第一次请求。', modelList: '模型列表', modelListDescription: '浏览可用模型和当前公开价格。', dashboard: 'API 控制台', dashboardDescription: '管理密钥、监控用量并查看费用。', accessTitle: '通过一个 API 使用主流模型', typesTitle: '按模型类型探索', startTitle: '几分钟完成接入', whyTitle: '为什么选择 OwnAPI', resourcesTitle: '更多资源', textTitle: '文本与大语言模型', textDescription: '通过一个 Chat Completions 地址使用 GPT、Claude、Gemini、DeepSeek 等模型。', imageTitle: '多模态与图像', imageDescription: '通过一致的认证层调用视觉理解与图像模型。', codeTitle: '代码与智能体', codeDescription: '无需分别管理不同提供商的密钥，即可构建编程助手和智能体。', baseUrlTitle: '只需更换地址与密钥', baseUrlDescription: '现有 OpenAI SDK 应用只需修改 baseURL 和 apiKey 即可使用 OwnAPI。', unifiedTitle: '一个密钥，多个模型', unifiedDescription: '保持同一套接入，通过模型 ID 切换不同提供商。', billingTitle: '透明的用量记录', billingDescription: '在 OwnAPI 工作台查看请求、Token 和费用。', reliabilityTitle: '生产环境控制', reliabilityDescription: '结合状态监控、重试和模型回退构建可靠应用。', errorsTitle: '错误与重试', errorsDescription: '按明确规则处理认证、限流和上游错误。', authErrorDescription: '检查 API Key 和 Authorization 请求头。', rateLimitDescription: '等待一段时间，并使用随机退避后重试。', upstreamErrorDescription: '重试临时上游故障，或切换到备用模型。', integrationsTitle: '集成指南', integrationsDescription: '连接 OpenAI 兼容 SDK、编辑器和自动化工具。', statusTitle: '模型状态', statusDescription: '排查应用问题前先确认模型可用性。', navGroups: { start: '从这里开始', api: '模型 API 快速开始', guides: '核心 API 指南', support: '支持' }, navItems: { overview: '文档概览', key: '创建 API Key', models: '列出模型', text: '文本 API', images: '图像 API', videos: '视频 API', sdk: '使用 OpenAI SDK', baseUrl: '更换 Base URL', fallback: '模型回退', errors: '处理错误', limits: '速率限制', help: '帮助中心' }, videoTitle: '视频生成 API', videoDescription: '通过同一套 OwnAPI 接口创建、轮询并下载 MiniMax H3 或 Wan 3.0 视频。客户只使用 OwnAPI Key，私有路由、上游凭据、真实任务 ID 与源视频地址全部保留在服务端。', videoModelLabel: '视频模型', videoCreateTitle: '创建任务', videoCreateDescription: '选择模型 ID 并发送 JSON，OwnAPI 会在服务端完成私有协议转换与路由。', videoCreateDescriptionH3: 'MiniMax-H3 支持 5–15 秒，以及 768p 或 2K 分辨率。HTTPS 地址使用 JSON，本地文件使用 multipart/form-data。', videoCreateDescriptionWan: 'wan3.0-video 与 wan3.0-video-prime 支持 2–30 秒，以及 480P、720P 或 1080P 分辨率。', videoUrlExampleTitle: 'HTTPS 地址 JSON', videoUploadExampleTitle: '本地文件 multipart', videoRetrieveTitle: '查询任务状态', videoRetrieveDescription: '使用返回的 OwnAPI 不透明任务 ID 轮询，直到状态变为 completed 或 failed。', videoDownloadTitle: '下载 MP4', videoDownloadDescription: '任务完成后通过 OwnAPI 下载二进制视频，并及时保存，因为上游结果只会保留有限时间。', videoInputsTitle: '参考输入', videoInputsDescription: '可在各模型限制内传入 HTTPS 地址、Base64 data URI 或 multipart 文件。', videoImagesHint: '最多 9 张 PNG/JPEG 参考图，每张不超过 20 MiB', videoVideosHint: '最多 3 段 MP4，每段不超过 50 MiB、时长 2–15 秒', h3VideoUnavailableHint: 'MiniMax H3 参考视频暂不可用；包含该字段的请求会在创建上游任务前被拒绝。', videoAudiosHint: '最多 3 段 MP3，每段不超过 15 MiB；音频必须搭配参考图', videoTotalInputsHint: '单次最多 12 个参考输入；同一媒体字段不能混用 URL 与文件。', wanImagesHint: '最多 10 张参考图', wanVideosHint: '最多 5 段；每段 1–15 秒且总时长不超过 15 秒', wanAudiosHint: '最多 5 段；每段 1–15 秒且总时长不超过 15 秒', wanControlsHint: 'Wan 画面比例、音频、随机种子、提示词改写和水印参数', videoFramesHint: '可选 PNG 首帧/尾帧文件或 HTTPS PNG 地址', videoExclusivityHint: '首帧/尾帧不能与参考图、参考视频、参考音频、文件或链接在同一请求中混用。'
  },
  ...
  admin: {
    ...
    users: {
      ...
      modelRates: '模型倍率',
      modelRatesTitle: '模型专属倍率',
      modelRatesHint: '为 {email} 设置按模型覆盖倍率。未填写的模型继承客户分组或分组默认倍率。',
      ownApiCustomerGroup: 'OwnAPI 客户分组',
      loadingModelRates: '正在加载模型倍率…',
      noConfigurableModels: '暂无可配置模型',
      inheritRate: '继承',
      clearRate: '清除',
      saveModelRates: '保存倍率',
      modelRatesSaved: '模型倍率已保存',
      failedToLoadModelRates: '加载模型倍率失败',
      failedToSaveModelRates: '保存模型倍率失败',
      invalidModelRate: '已填写的倍率必须大于 0',
      replaceGroup: '替换分组',
    ...
    settings: {
      ...
      payment: {
        ...
        maxAmount: '最高金额',
        dailyLimit: '每日限额',
        balanceRechargeMultiplier: '换算后余额倍率',
        balanceRechargeMultiplierHint: 'CNY 按 6.7:1、USD 按 1:1 换算后再应用的额外倍率，标准值为 1.0',
        balanceRechargePreview: '预览：支付 6.70 CNY 或 1.00 USD，到账 {usd} USD',
