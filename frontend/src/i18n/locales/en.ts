export default {
  ...
  usage: {
    ...
    pricingEffectiveAt: 'Pricing effective at (UTC)',
    original: 'Original',
    billed: 'Billed',
  ...
  publicModels: {
    ...
    searchLabel: 'Search the model catalog',
    ...
    sortInputPrice: 'Lowest input price',
    sortOutputPrice: 'Lowest output price',
    featured: 'Featured',
    from: 'From',
    ...
    videoPriceEstimate: 'Video is billed by generated duration. The table shows the official list and OwnAPI price for each model and resolution.',
    viewProtocol: 'View API protocol',
    videoGenerator: { eyebrow: 'Signed-in users', title: 'Generate a {model} video', apiKey: 'OwnAPI API key', prompt: 'Prompt', promptPlaceholder: 'Describe the scene, motion, camera and style…', duration: 'Duration', resolution: 'Resolution', ratio: 'Aspect ratio', seed: 'Random seed', audio: 'Generate audio', promptExtend: 'Rewrite prompt', watermark: 'Add watermark', referenceUrl: 'Reference image URL (optional)', referenceFile: 'Upload reference image (optional)', firstFrameUrl: 'First frame URL (optional)', firstFrameFile: 'Upload first frame (optional)', lastFrameUrl: 'Last frame URL (optional)', lastFrameFile: 'Upload last frame (optional)', referenceVideoUrl: 'Reference video URL (optional)', referenceVideoFile: 'Upload reference video (optional)', referenceAudioUrl: 'Reference audio URL (optional)', referenceAudioFile: 'Upload reference audio (optional)', h3ReferenceVideoUnavailable: 'MiniMax H3 reference-video input is temporarily unavailable. Reference images remain supported.', h3VideoDuration: 'MiniMax H3 reference videos must be 2–15 seconds long.', invalidH3Video: 'The MiniMax H3 reference-video duration could not be read.', submit: 'Generate video', submitting: 'Creating task…', status: 'Task status', download: 'Open or download video', failed: 'Video request failed' },
    ...
    pricingCondition: {
      ...
      baseOutsidePeak: 'The base OwnAPI prices shown above apply outside peak hours.',
      schedule: 'Peak schedule',
      weekdays: 'Monday–Friday',
      beijingTime: 'Beijing {windows}',
      utc: 'UTC equivalent',
      multiplier: 'Peak multiplier',
      halfOpen: 'Opening times are included; closing times are excluded.',
      usageAudit: 'Usage history records whether the standard 1× or peak 2× multiplier applied.',
    },
    ...
    families: {
      ...
      wan: { summary: 'Alibaba Wan multimodal video generation.', description: 'High-quality video workflows driven by text, first and last frames, reference images, video and audio.' },
  ...
  publicDocs: {
    ...
    videoTitle: 'Video generation API', videoDescription: 'Create, poll and download MiniMax H3 or Wan 3.0 videos through the same OwnAPI gateway. Customers only use an OwnAPI key; private routing, credentials, task IDs and source URLs remain server-side.', videoModelLabel: 'Video model', videoCreateTitle: 'Create a task', videoCreateDescription: 'Choose a model ID and send JSON. OwnAPI privately translates and routes the request.', videoCreateDescriptionH3: 'Use MiniMax-H3 with a 5–15 second duration and 768p or 2K resolution. Use JSON for direct HTTPS URLs or multipart form data for local files.', videoCreateDescriptionWan: 'Use wan3.0-video or wan3.0-video-prime with a 2–30 second duration and 480P, 720P or 1080P resolution.', videoUrlExampleTitle: 'HTTPS URL JSON', videoUploadExampleTitle: 'Local file multipart', videoRetrieveTitle: 'Retrieve task status', videoRetrieveDescription: 'Poll with the returned opaque OwnAPI task ID until status is completed or failed.', videoDownloadTitle: 'Download the MP4', videoDownloadDescription: 'When the task is completed, download the binary video from OwnAPI. Save it promptly because provider result retention is limited.', videoInputsTitle: 'Reference inputs', videoInputsDescription: 'Pass direct HTTPS URLs, Base64 data URIs, or multipart file uploads within each model’s limits.', videoImagesHint: 'up to 9 PNG/JPEG references, 20 MiB each', videoVideosHint: 'up to 3 MP4 references, 50 MiB each and 2–15 seconds each', h3VideoUnavailableHint: 'MiniMax H3 reference-video input is temporarily unavailable; requests containing it are rejected before an upstream task is created.', videoAudiosHint: 'up to 3 MP3 references, 15 MiB each; audio requires an image', videoTotalInputsHint: 'At most 12 reference inputs in one request. Do not mix a URL and a file in the same media field.', wanImagesHint: 'up to 10 reference images', wanVideosHint: 'up to 5 clips, each 1–15 seconds and no more than 15 seconds total', wanAudiosHint: 'up to 5 clips, each 1–15 seconds and no more than 15 seconds total', wanControlsHint: 'Wan aspect ratio, audio, random seed, prompt rewriting and watermark controls', videoFramesHint: 'optional PNG first/last frame uploads or direct HTTPS PNG URLs', videoExclusivityHint: 'Use first/last frames instead of reference images, videos, audio, file or link inputs in the same request.'
  },
  ...
  admin: {
    ...
    users: {
      ...
      modelRatesTitle: 'Per-Model Rates',
      modelRatesHint: 'Set per-model rate overrides for {email}. Empty models inherit the user group or group default rate.',
      ownApiCustomerGroup: 'OwnAPI Customer Group',
      loadingModelRates: 'Loading model rates...',
      noConfigurableModels: 'No configurable models',
      inheritRate: 'Inherit',
      clearRate: 'Clear',
      saveModelRates: 'Save Rates',
      modelRatesSaved: 'Model rates saved',
      failedToLoadModelRates: 'Failed to load model rates',
      failedToSaveModelRates: 'Failed to save model rates',
      invalidModelRate: 'Every configured rate must be greater than 0',
    ...
    settings: {
      ...
      payment: {
        ...
        balanceRechargeMultiplier: 'Post-conversion Balance Multiplier',
        balanceRechargeMultiplierHint: 'Extra multiplier applied after CNY is converted at 6.7:1 or USD at 1:1. Standard value: 1.0',
        balanceRechargePreview: 'Preview: 6.70 CNY or 1.00 USD credits {usd} USD',
  ...
  payment: {
    ...
    rechargeRatePreview: 'CNY is converted to USD at 6.7:1; fees are not credited',
