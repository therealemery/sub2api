export default {
  ...
  publicDocs: {
    ...
    videoTitle: 'MiniMax H3 video API', videoDescription: 'Create, poll and download MiniMax H3 videos through the OwnAPI gateway. Customers only use an OwnAPI key; upstream provider credentials stay private.', videoCreateTitle: 'Create a task', videoCreateDescription: 'Use the MiniMax-H3 model ID. The request accepts JSON and is converted privately to the upstream media request.', videoRetrieveTitle: 'Retrieve task status', videoRetrieveDescription: 'Poll with the returned task ID until status is completed or failed.', videoDownloadTitle: 'Download the MP4', videoDownloadDescription: 'When the task is completed, download the binary video from the content endpoint.', videoInputsTitle: 'Reference inputs', videoInputsDescription: 'Reference media URLs or data URIs can be combined within the provider limits.', videoImagesHint: 'up to 9 PNG/JPEG references', videoVideosHint: 'MP4 references, 2–15 seconds each', videoAudiosHint: 'MP3 references, pair audio with an image', videoFramesHint: 'optional first/last frame data or URLs', videoExclusivityHint: 'Use first/last frames instead of reference images, videos or audio in the same request.'
  },
