...
describe('modelCatalog', () => {
  ...
  it('does not let stale live display configuration republish DeepSeek Pro', () => {
    const catalog = buildModelCatalog({
      featured_models: [{ model: 'deepseek-v4-pro', platform: 'deepseek', badge: 'Featured', sort_order: 1 }],
      reference_discount: null,
      pricing_models: [{
        model: 'deepseek-v4-pro',
        platform: 'deepseek',
        billing_mode: 'token',
        input_price: 0.000000105,
        output_price: 0.00000042,
        cache_write_price: null,
        cache_read_price: 0.0000000021,
        image_output_price: null,
        per_request_price: null,
        sort_order: 1,
      }],
    ...
    expect(catalog.some((entry) => entry.modelId === 'deepseek-v4-pro')).toBe(false)
  })
  ...
  it('contains the complete verified catalog with traceable discounted pricing', () => {
    ...
    expect(new Set(requiredIds).size).toBe(17)
    ...
    const reviewedOnSeptember13 = new Set([
      'gpt-6-astra', 'gpt-5.4-mini', 'gpt-5.5', 'gpt-5.6-luna',
      'gpt-5.6-sol', 'gpt-5.6-terra', 'codex-auto-review', 'deepseek-v4.1-flash',
    ])
    for (const modelId of requiredIds) {
      ...
      expect(model?.pricingSource).toMatchObject({
        ...
        checkedAt: reviewedOnSeptember13.has(modelId) ? '2026-09-13' : '2026-09-07',
    ...
    expect(catalog.find((model) => model.modelId === 'gpt-6-astra')?.pricingSource?.tiers).toEqual([{
      id: 'long', minInputTokens: 272_000, minInclusive: false, maxInputTokens: null, maxInclusive: true,
      official: { input: 20, cachedInput: 2, cacheWrite: 25, output: 75 },
    }])
    const deepSeekPricing = catalog.find((model) => model.modelId === 'deepseek-v4.1-flash')?.pricingSource
    expect(deepSeekPricing?.multiplier).toBe(0.7)
    const deepSeekOwnApi = calculateOwnApiPricing(deepSeekPricing!.official, deepSeekPricing!.multiplier)
    expect(deepSeekOwnApi.input).toBeCloseTo(0.105, 12)
    expect(deepSeekOwnApi.cachedInput).toBeCloseTo(0.0021, 12)
    expect(deepSeekOwnApi.cacheWrite).toBeNull()
    expect(deepSeekOwnApi.output).toBeCloseTo(0.42, 12)
    expect(catalog.find((model) => model.modelId === 'gpt-5.4-mini')?.pricingSource?.multiplier).toBe(0.8)
    expect(catalog.find((model) => model.modelId === 'gpt-5.5')?.pricingSource?.multiplier).toBe(0.8)
    expect(catalog.find((model) => model.modelId === 'codex-auto-review')?.pricingSource?.multiplier).toBe(0.8)
  ...
  it('contains the exact 42-model eligibility snapshot across nine providers', () => {
    ...
    for (const entry of catalog) {
      expect(entry.eligibilitySource?.discountPercent).toBeGreaterThanOrEqual(20)
