...
describe('MiniMaxVideoGenerator', () => {
  ...
  it.each([
    ...
    { index: 3, name: 'reference.mp3', type: 'audio/mpeg', field: 'reference_audios', content: new Uint8Array([1]) },
  ...
  it('hides H3 reference-video controls while keeping the reference-image input', () => {
    const wrapper = mount(MiniMaxVideoGenerator, {
      props: { modelId: 'MiniMax-H3', pricing: [{ resolution: '768p', ownApiPerSecond: 0.05597 }] },
    })
    expect(wrapper.find('input[placeholder="https://…/reference.mp4"]').exists()).toBe(false)
    expect(wrapper.find('input[accept="video/mp4,video/*"]').exists()).toBe(false)
    expect(wrapper.get('.input-notice').text()).toBe('publicModels.videoGenerator.h3ReferenceVideoUnavailable')
