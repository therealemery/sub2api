<template>
  ...
  >
    ...
    >
      <div class="text-sm text-gray-600 text-[var(--text-muted)]">
        {{ t('admin.accounts.syncFromCrsDesc') }}
      </div>
      <div
        class="rounded-lg bg-gray-50 p-3 text-xs text-gray-500 bg-[var(--bg-surface-alt)] text-[var(--text-muted)]"
      >
      ...
      <div class="grid grid-cols-1 gap-4">
        ...
        <label class="flex items-center gap-2 text-sm text-gray-700 text-[var(--text-muted)]">
          <input
            v-model="form.sync_proxies"
            type="checkbox"
            class="rounded border-gray-300 border-[var(--border-default)]"
          />
    ...
    <div v-else-if="currentStep === 'preview' && previewResult" class="space-y-4">
      ...
      <div
        ...
        class="rounded-lg bg-gray-50 p-3 bg-[var(--bg-surface-alt)]"
      >
        <div class="mb-2 text-sm font-medium text-gray-700 text-[var(--text-muted)]">
          {{ t('admin.accounts.crsExistingAccounts') }}
          <span class="ml-1 text-xs text-gray-400">({{ previewResult.existing_accounts.length }})</span>
        </div>
        <div class="max-h-32 overflow-auto text-xs text-gray-500 text-[var(--text-muted)]">
          <div
          ...
          >
            <span
              class="inline-block rounded bg-[var(--bg-surface-alt)] px-1.5 py-0.5 text-[10px] font-medium text-[var(--accent)] bg-[var(--bg-surface-alt)] text-[var(--accent)]"
      ...
      <div v-if="previewResult.new_accounts.length">
        ...
        <div
          class="max-h-48 overflow-auto rounded-lg border border-gray-200 p-2 border-[var(--border-default)]"
        >
          <label
          ...
          >
            ...
            >{{ acc.platform }} / {{ acc.type }}</span>
            <span class="truncate text-sm text-gray-700 text-[var(--text-muted)]">{{ acc.name }}</span>
          </label>
        </div>
      ...
      <div class="flex items-center gap-2 text-xs text-gray-500 text-[var(--text-muted)]">
        <span>{{ t('admin.accounts.syncProxies') }}:</span>
        <span :class="form.sync_proxies ? 'text-green-600 dark:text-green-400' : 'text-gray-400 text-[var(--text-muted)]'">
      ...
      <div
        ...
        class="rounded-lg bg-gray-50 p-4 text-center text-sm text-gray-500 bg-[var(--bg-surface-alt)] text-[var(--text-muted)]"
      >
    ...
    <div v-else-if="currentStep === 'result' && result" class="space-y-4">
      ...
      >
        ...
        <div v-if="errorItems.length" class="mt-2">
          ...
          <div
            class="mt-2 max-h-48 overflow-auto rounded-lg bg-gray-50 p-3 font-mono text-xs bg-[var(--bg-surface-alt)]"
          >
