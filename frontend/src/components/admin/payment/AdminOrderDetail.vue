    ...
    <div v-if="order" class="space-y-4">
      <div class="grid grid-cols-2 gap-4">
        ...
        <div v-if="order.amount !== order.pay_amount">
          ...
          <p class="text-sm font-medium text-gray-900 dark:text-white">{{ formatPaymentAmount(order.amount, order.order_type === 'balance' ? 'USD' : order.currency) }}</p>
        </div>
        <div>
      ...
      >
        ...
        <div class="grid grid-cols-2 gap-2 text-sm">
          <div>
            ...
            <span class="ml-1 font-medium text-red-700 dark:text-red-300">{{ formatPaymentAmount(order.refund_amount, order.order_type === 'balance' ? 'USD' : order.currency) }}</span>
          </div>
