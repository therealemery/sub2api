...
- Changed `LayoutView.vue` to use the router-view slot API and the stable `route.fullPath` key around ordinary authenticated user pages.
- Wrapped public page content in an appearing `motion-fade` transition keyed by `route.fullPath`, while leaving the existing public header, footer, authentication check, and public-settings loading intact.
...
Coverage includes the keyed router-view slot, ordinary-user transition name/mode, direct rendering for `/admin`, `/admin/dashboard`, and `/administrator-preview`, public-route fade configuration, mobile-menu transition name, `aria-expanded`, keyboard focusability, and immediate link-close behavior.
...
- Public views continue to own `PublicSiteLayout`; `appear` ensures their route content fades in whenever a public route mounts.
