...
var usageLogInsertArgTypes = [...]string{
	...
	"timestamptz", // pricing_effective_at
	"numeric",     // condition_multiplier
	"text",        // pricing_rule_id
...
func prepareUsageLogInsert(log *service.UsageLog) usageLogInsertPrepared {
	...
	var pricingEffectiveAt sql.NullTime
	if log.PricingEffectiveAt != nil && !log.PricingEffectiveAt.IsZero() {
		pricingEffectiveAt = sql.NullTime{Time: log.PricingEffectiveAt.UTC(), Valid: true}
	}
	conditionMultiplier := log.EffectiveConditionMultiplier()
	pricingRuleID := nullString(log.PricingRuleID)
...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		pricingEffectiveAt    sql.NullTime
		conditionMultiplier   float64
		pricingRuleID         sql.NullString
		createdAt             time.Time
	)
	...
	if pricingEffectiveAt.Valid {
		value := pricingEffectiveAt.Time.UTC()
		log.PricingEffectiveAt = &value
	}
	if pricingRuleID.Valid {
		value := pricingRuleID.String
		log.PricingRuleID = &value
	}
