# ownapi Deployment Files

This directory contains files for deploying ownapi on Linux servers.
...
curl -sSL https://raw.githubusercontent.com/your-org/ownapi/main/deploy/docker-deploy.sh | bash

# Or download first, then run
curl -sSL https://raw.githubusercontent.com/your-org/ownapi/main/deploy/docker-deploy.sh -o docker-deploy.sh
...
# View logs
docker compose -f docker-compose.local.yml logs -f ownapi

# If admin password was auto-generated, find it in logs:
docker compose -f docker-compose.local.yml logs ownapi | grep "admin password"
...
```bash
# Clone repository
git clone https://github.com/your-org/ownapi.git
cd ownapi/deploy
...
3. If `ADMIN_PASSWORD` is not set, check logs for the generated password:
   ```bash
   docker compose logs ownapi | grep "admin password"
   ```
...
# Restart ownapi only
docker compose -f docker-compose.local.yml restart ownapi
...
docker compose -f docker-compose.local.yml down
cd ..
tar czf ownapi-complete.tar.gz deployment/

# Transfer to new server
scp ownapi-complete.tar.gz user@new-server:/path/to/destination/

# On new server: Extract and start
tar xzf ownapi-complete.tar.gz
cd deployment/
docker compose -f docker-compose.local.yml up -d
```
...
ownapi supports three methods to connect to Gemini:
...
```bash
curl -sSL https://raw.githubusercontent.com/your-org/ownapi/main/deploy/install.sh | sudo bash
```

### Manual Installation

1. Download the latest release from [GitHub Releases](https://github.com/your-org/ownapi/releases)
...
# View detailed logs
docker compose -f docker-compose.local.yml logs --tail=100 ownapi
...
# View detailed logs
docker compose logs --tail=100 ownapi
...
ownapi supports TLS fingerprint simulation to make requests appear as if they come from the official Claude CLI (Node.js client).
