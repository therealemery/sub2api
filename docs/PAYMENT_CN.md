...
用户余额统一以 USD 计价，余额充值仅支持使用 CNY 或 USD 的支付实例：

- 支付本金 `6.70 CNY` 到账 `1.00 USD`；支付本金 `1.00 USD` 到账 `1.00 USD`。
- `BALANCE_RECHARGE_MULTIPLIER` 在币种换算后应用，标准汇率请保持为 `1.0`。
- 支付手续费只增加 `pay_amount`，不计入到账余额。
- 余额订单的 `amount` 保存到账 USD，`pay_amount` 保存支付渠道币种的实付金额，支付币种保存在服务商快照中。
- 订阅订单金额、Webhook 实付金额校验以及全额/按比例退款语义保持不变。
